...
import { addToCart, addToQuantity, decreaseQuantity, deleteProductFromCart } from '../../redux/actions/ShoppingCartAction'
...
const index = () => {
    ...
    const shoppingCart = useSelector(state=>state.shoppingCart)
        ...
        const availableProduct = shoppingCart.find(item=>item.id===product[0].id) 
        let productCount = 0
           if (availableProduct) {
...

        {
            product.map((productInformation, index) => {
                return (
                   ...
                   {
                     displayStatus && <ProductSecondImageDisplay images={productInformation}/>
                   } 
                    ...
                    <div className={style.sellerAndPriceContainer} >
                       ...
                       <div>
                        ...
                        <span className={style.storeName}>{productInformation.sellerName}</span>
                       </div>
                       <div>
                        <BsShieldCheck className={style.guaranteeIcon}/>
                        <p className={style.guaranteeInfoText}>{productInformation.guaranteeInfo}</p>
                       </div>
                       <div>
                      
                      <>
                        <BsClipboardCheck className={style.checkIcon}/>
                        <span className={style.checkText}>موجود در انبار فروشنده</span>
                      </>
                       </div>
                       <div className={style.priceInfoContainer}>
                              <span>قیمت فروشنده :</span>
                              <span>{productInformation.price_after.toLocaleString('en-US')} تومان</span>
                       </div>

                            
                         
                         {productCount>0?<div style={{justifyContent:'space-evenly'}}>
                          <div className={style.incOrDecButtonSection}>
                             <button className={style.incOrDecButton} onClick={addProductQuantity}>+</button>
                          
                          <span>{productCount}</span>
                         { productCount === 1 ? <VscTrash className={style.incOrDecButton} onClick={deleteProduct}/>
                          :null} {productCount>1?<button className={style.incOrDecButton} onClick={decProductQuantity}>-</button>:null}
                          </div>

                          <div className={style.btnText}>
                          <span>در سبد شما مشاهده در</span>
                          <LinkButton href='/shoppingCart' value='سبد خرید' linkClassname={style.navBtn}/>
                          </div>
                      
                          </div>:<AddToCartButton className={style.addToCartButton} func={clickHandler} value={'افزودن به سبد'}/> }
